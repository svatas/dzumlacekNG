	...
	public partial class MainForm : Form
	{
        ...
        private void simpleOpen(string strProcess, string strArgs)
        {
        	try
        	{
        		...
        		Process.Start(strProcess, strArgs);
        	}
        	catch (Exception)
        	{
		...
		private void MainFormFormClosing(object sender, FormClosingEventArgs e)
		{
        	...
        	if (e.CloseReason== CloseReason.UserClosing && listMP != null)
        	{
        		foreach (MonitoredProcess mp in listMP)
        		{
        			if (mp.isAlive()) e.Cancel = true;
        		}
        	}
        	// Else if the computer is closing the form, force kill all the apps
		...
		private void killAll()
		{
			...
			{
				if (listMP[i].isAlive()) listMP[i].kill();
				listMP.RemoveAt(i);
			}
		}
		...
		{
			simpleOpen(@"http://www.josephspurrier.com");
		}
		...
		{
			simpleOpen(@"https://github.com/josephspurrier");
		}
		
		private void MongoDBToolStripMenuItemClick(object sender, EventArgs e)
		{
			simpleOpen(var.strMongoDBFolderPath);
		}
