...
namespace DzumlacekNG
{
	...
	public class MPApache : MonitoredProcess
	{
		...
		protected override bool configureApp()
		{
			...
			// Important comment - When upgrading PHP version, be sure to change proper dll name changes
			...
			alReplace.Add(new string[]{"#%PHPMODULE%", "LoadModule php7_module \""
			              		+ Path.Combine(var.strPHPTSFolderPath,"php7apache2_4.dll").Replace('\\','/') + "\""});
